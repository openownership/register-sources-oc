    ...
    it 'scrolled results' do
      expect(es_client).to receive(:search).with(
      ...
      ).and_return(
        {
          ...
          'hits' => {
            'hits' => [
              { '_source' => { jurisdiction_code: 'sk', company_number: 'C1', identifier_system_code: 'lei',
                               uid: 'X-C1' } }
            ]
          }
        }
      ...
      expect(es_client).to receive(:scroll).with(
      ...
      ).and_return(
        {
          'hits' => {
            'hits' => [
              { '_source' => { jurisdiction_code: 'dk', company_number: 'C2', identifier_system_code: 'lei',
                               uid: 'X-C2' } }
            ]
          }
        }
      ...
      expect(es).to eq(
        [
          RegisterSourcesOc::AddId.new(jurisdiction_code: 'sk', company_number: 'C1', identifier_system_code: 'lei',
                                       uid: 'X-C1'),
          RegisterSourcesOc::AddId.new(jurisdiction_code: 'dk', company_number: 'C2', identifier_system_code: 'lei',
                                       uid: 'X-C2')
        ]
      )
    end

    it 'filtered jurisdiction' do
      expect(es_client).to receive(:search).with(
        {
          index:,
          body: {
